...
Сразу после выполнения каждого пункта проверь результат. Если этап связан с
...
Для внутреннего пункта достаточно `--evidence`. Не откладывай галки до конца
этапа или финального ответа. Повтор с тем же evidence/read-back идемпотентен;
другая попытка переписать уже завершённый item отклоняется.
