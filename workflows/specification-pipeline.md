...
2. Если его нет, инициализируй `compact` case через `case_pipeline.py` в
   рабочей области координатора, не в каноническом каталоге проекта.
...
**Выход:** `manifest.json`, `ledger.json`, `kernel.md` и следующая фаза определены.
