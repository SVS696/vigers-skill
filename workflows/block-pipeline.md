...
5. Консолидируй diagram surfaces всех блоков. Удали дубли, но не теряй вопросы;
...
6. После фактического включения переводите блоки `reviewed → integrated`.
7. Зафиксируй гейт `semantic_integration` с evidence `draft.md`.
8. Обнови working projection интегрированным draft, сохраняя явную маркировку
...
2. Исправь дубликаты ID, неразрешённые ссылки, stale-блоки и разрывы
   `REQ ↔ AC`.
3. Сверь матрицу диаграмм: каждый указанный source ID существует, а каждый
   required-вопрос имеет определённое размещение и render target.
4. Повторяй ту же команду до `PASS`; не закрывай гейт вручную.
